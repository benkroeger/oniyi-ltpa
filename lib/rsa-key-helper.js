...
const bigIntFromBuf = buf =>
  BigInteger(BigIntegerBuffer.fromBuffer(1, buf).toString());
const bufFromBigInt = bigInt =>
  BigIntegerBuffer.fromString(bigInt.toString(10), 10).toBuffer();
...
const LTPAPrivKeyToComponentBuffers = inputBuffer => {
  ...
  if (inputBuffer.length > eLength + pLength + qLength) {
    ...
    inputBuffer.copy(
      components[2],
      0,
      4 + privExponentLength,
      4 + privExponentLength + eLength,
    );
    inputBuffer.copy(
      components[3],
      0,
...
const compileMissingRSAComponents = components => {
  ...
  const abiginteger = components.map(
    val => (Buffer.isBuffer(val) && bigIntFromBuf(val)) || null,
  );
  ...
  if (abiginteger[5] == null) {
    abiginteger[5] = abiginteger[1].remainder(
      abiginteger[3].subtract(BigInteger.one),
    );
  }
  if (abiginteger[6] == null) {
    abiginteger[6] = abiginteger[1].remainder(
      abiginteger[4].subtract(BigInteger.one),
    );
  }
...
const bufferToKeyComponents = buffer =>
  compileMissingRSAComponents(LTPAPrivKeyToComponentBuffers(buffer));
...
const bufferToRSAPrivateKeyPEM = buffer =>
  bufferToRSA(buffer).exportKey('private');

module.exports = {
  bufferToKeyComponents,
  bufferToRSA,
  bufferToRSAPrivateKeyPEM,
};
