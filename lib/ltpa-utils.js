...
const {
  VERSION, DES3_KEY, PRIVATE_KEY, REALM, CREATION_DATE, CREATION_HOST, PUBLIC_KEY,
} = require('./constants');
...
const readKeyfile = (filePath, cb) => {
  properties.parse(path.resolve(process.cwd(), filePath), { path: true }, (error, propertiesFileContent) => {
    if (error) {
      cb(error);
      return;
    }

    const keyFileContents = Object.keys(keyFilePropsMap).reduce((result, key) => {
...
const makeLtpaBodyObject = body =>
  body.split('$').reduce((result, entry) => {
    ...
    return Object.assign(result, { [name]: Array.isArray(vals) ? vals.join(':') : vals });
  }, {});

const makeLtpaBodyString = body =>
  Object.keys(body)
...
const getLtpaTokenContent = (encryptedLtpaToken, key) => {
  ...
  const decryptedLtpaTokenParts = decryptedLtpaTokenBuffer.toString().split('%');

  return decryptedLtpaTokenParts.reduce((result, val, idx) => Object.assign(result, { [idxToNameMap[idx]]: val }), {});
};
...
const getLtpaToken2Content = (encryptedLtpaToken2, key) => {
  ...
  const decryptedLtpaToken2Parts = decryptedLtpaToken2Buffer.toString().split('%');

  return decryptedLtpaToken2Parts.reduce((result, val, idx) => Object.assign(result, { [idxToNameMap[idx]]: val }), {});
};
...
const ensureExpires = (body, expire) => Object.assign({}, body, { expire });
...
const makeLtpaToken2 = ({ body, expires }, signingKey, encryptionKey) => {
  const bodyString = typeof body === 'string' ? body : makeLtpaBodyString(ensureExpires(body, expires));
